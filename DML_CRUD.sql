...
, IDEMPA IN INTEGER 
, CANTIDAD IN INTEGER 
, PRECIOUNI IN INTEGER
) AS 
ValorIvaProd FLOAT(10);
BEGIN
  SELECT ValorIva INTO ValorIvaProd FROM IvaProductos;
  INSERT INTO PRODUCTOS (idproducto,nombre_producto,descripcion_producto, peso_producto, catalogo_idcategoria, tempaque_idtempaque,cantidad_total,precioUnitario,iva)
    VALUES (IDPROD,NOMBRE,DESCRIP,PESO,IDCAT,IDEMPA,CANTIDAD,PRECIOUNI,PRECIOUNI*ValorIvaProd);
...
MONTOIVA INTEGER;
BEGIN
  SELECT precioUnitario INTO PRECIOUNI FROM productos WHERE idproducto = IDPROD;
  SELECT Iva INTO MONTOIVA FROM productos WHERE idproducto = IDPROD;
  INSERT INTO PRODXCATALOGO (idprodcatalogo,precio_catalogo,cantidad_catalogo, catalogo_idcatalogo, productos_idproducto)
    VALUES (IDPRODCATA,PRECIOUNI + MONTOIVA,CANTCAT,IDCAT,IDPROD);
...
BEGIN
    INSERT INTO factura(idfactura, fecha_factura, fecha_entrega, pedidos_idpedido,valorTotalFacturaIVA,valorTotalFactura)
    VALUES(SEQUENCE_FACTURA.nextval , to_char(sysdate,'dd-mon-yyyy'), to_char(sysdate + 8,'dd-mon-yyyy'), IDPED, null, null);
